...
var name, code, players = {}, me = {}, gameStarted = false, gameSortaStarted = false, left = false, right = false,down = false,lap;
var carPos = [
	{x: 0, y: 0},
	{x: 2, y: 0},
	{x: -2, y: 0},
...
menu2 = function(){
	...
	setTimeout(function(){
		f.innerHTML = "<div class='menuitem title button' id='host' ontouchstart='this.click()' onclick='host()'>Crear servidor</div><div class='menuitem title button' ontouchstart='this.click()' id='join' onclick='joinGame()'>Unirse a un servidor</div>";
...
host = function(){
	...
	f.style.transform = "translate3d(0, -100vh, 0)";
	setTimeout(function(){
		f.innerHTML = "<div class='info title'>Código del servidor!<div id='code'>Cargando...</div></div><div id='startgame' class='title' onclick='startGame()' ontouchstart='this.click()'>Start!</div>";
		if(VR)
...
joinGame = function(){
	...
	f.style.transform = "translate3d(0, -100vh, 0)";
	setTimeout(function(){
		f.innerHTML = "<div class='info title'>Ingresa el código para unirte!<input id='incode' class='title' onkeyup='codeCheck(event)' ontouchstart='this.focus()'></input></div>";
		if(VR)
			f.innerHTML += "<div id='divider'></div>";
		f.appendChild(element);
		f.style.transform = "none";
			...
			if(!gameSortaStarted){
				for(var p in players){
					...
					if(play.data.lap > 3){
						...
						document.getElementById("countdown").innerHTML = play.data.name + " Gano!";
					}
	...
	if(incode.value.length == 4){
		...
		database.ref(code).once("value", function(cc){
			if(typeof cc.val() != "undefined" && cc.val() != null && cc.val().status === 0){
				document.getElementsByClassName("info")[0].innerHTML = "<div class='info title'>Esperando a los jugadores...<div id='code'>" + code + "</div></div>";
				var playerCount = 0;
